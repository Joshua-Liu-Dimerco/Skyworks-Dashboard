...
# Dispatch rules: customer key (uppercase substring) → {destination key (uppercase): rule}
# Destination matched against "Ship To Country" column
RULES_RAW = {
...
def process_data(df: pd.DataFrame):
    """Apply all business logic and return (df, customer_col)."""
    today = date.today()
    ...
    # ── Step 2: Work Status (needed for SP check in cutoff) ──
    def work_status(row):
    ...
    # ── Step 3: Dispatch rule (needed for cutoff check) ──
    ...
    # ── Step 4: KPI date with 11:30 cutoff ──
    # Rule: If New CRSD = today AND SP = GO AND dispatch_today = True
    #       AND DN Created Time >= 11:30 → push to next working day
    def next_workday(d: date) -> date:
        nxt = d + timedelta(days=1)
        while nxt.weekday() >= 5:   # skip Saturday(5) / Sunday(6)
    ...
    def kpi_date_fn(row):
        ...
        if pd.isna(crsd):
            return crsd
        # 11:30 cutoff only applies when:
        # 1. CRSD is today  2. order is GO (not X)  3. dispatch_today  4. DN time >= 11:30
        is_go          = row["work_status"] == "已包待出_GO"
        d_today        = row["dispatch_today"]
        crsd_is_today  = (crsd == today)
        past_cutoff    = (
            pd.notna(dn)
            and (dn.hour, dn.minute) >= (CUTOFF_HOUR, CUTOFF_MINUTE)
        )
        if crsd_is_today and is_go and d_today and past_cutoff:
            return next_workday(today)
        return crsd

    df["kpi_date"] = df.apply(kpi_date_fn, axis=1)
    df["days_to_kpi"] = df["kpi_date"].apply(
        lambda d: (d - today).days if pd.notna(d) else 999
    )

    # ── Step 5: Effective ship date — next valid dispatch weekday from kpi_date ──
    def rule_weekdays(rule: str):
        r = str(rule).strip()
        if not r or r in ("", "Everyday", "FEDEX", "DHL", "T3EX"):
            return None   # every day OK
        days = []
        for t in [x.strip() for x in r.split("/")]:
            if t.startswith("W") and t[1:].isdigit():
                days.append(int(t[1:]) - 1)   # W1=Mon=0
        return days or None

    def next_dispatch_day(base: date, rule: str) -> date:
    ...
    def effective_ship_date(row) -> date:
        kd   = row["kpi_date"]
    ...
    df["effective_ship_date"] = df.apply(effective_ship_date, axis=1)
    df["days_to_effective"] = df["effective_ship_date"].apply(
        lambda d: (d - today).days if pd.notna(d) else 999
    )

    # ── Priority (based on original days_to_kpi for urgency) ──
    def priority(row):
        days  = row["days_to_kpi"]
        picked = "C - Completely" in str(row.get("Picking Status", ""))
        if days <= 0:                              return "P1"
        if days <= 2:                              return "P2"
        if row["dispatch_today"] and not picked:   return "P2"
        if days <= 5:                              return "P3"
        return "P4"
...
if page == "📊 Dashboard":
    ...
    st.markdown(
        ...
        f'<span style="font-size:15px;font-weight:600;color:#A32D2D;">今天必出 — New CRSD ≤ 今日</span>'
        f'<span style="font-size:26px;font-weight:600;color:#A32D2D;margin-left:auto;">{today_total}</span>'
    ...
    for col, label, val, sub, action, bg, fg in [
        ...
        with col:
            st.markdown(
                ...
                f'border-radius:6px;display:inline-block;font-weight:500;">{action}</div></div>',
                unsafe_allow_html=True,
            )
    ...
    for col, label, days_range, color in [
        ...
        with col:
            mask = (df["days_to_effective"] >= days_range[0]) & (df["days_to_effective"] <= days_range[1])
            sub  = df[mask]
            n    = len(sub)
            ...
            st.markdown(
                ...
                f'<div style="margin-top:4px;">{detail_str}</div></div>',
                unsafe_allow_html=True,
            )
...
# ══════════════════════════════════════════════
    ...
    if sel_bucket:  mask &= df["kpi_bucket"].isin(sel_bucket)
    if sel_cust:    mask &= df["customer_display"].isin(sel_cust)
    if sel_dispatch: mask &= df["dispatch_today"]
    if sel_x_only:
        sp_series = df.get("Special Processing", pd.Series([""] * len(df), index=df.index))
        mask &= sp_series.astype(str).str.strip() == "X"
    ...
    styled = filtered[display_cols].style.map(color_ws, subset=["work_status"]).map(color_p, subset=["priority"])
...
# ══════════════════════════════════════════════
    ...
    with c2:
        cust_opts    = sorted(df["customer_display"].dropna().unique())
    ...
    if sel_ws_d:              mask_d &= df["work_status"].isin(sel_ws_d)
    if sel_cust_d:            mask_d &= df["customer_display"].isin(sel_cust_d)
    if sel_sp_d and sp_col:   mask_d &= df[sp_col].isin(sel_sp_d)
    ...
    box_col    = find_col(df, "件數", "box", "carton")
    weight_col = find_col(df, "weight", "wt")
    dn_col     = find_col(df, "delivery")

    detail_cols = [c for c in [
        sp_col, dn_col, "customer_display",
        "New CRSD", "kpi_date", "effective_ship_date", "days_to_kpi", "kpi_bucket",
    ...
    st.dataframe(sub_d[detail_cols].style.apply(highlight_rows, axis=1),
...
# ══════════════════════════════════════════════
    ...
    candidate_cols = [
        ...
        "customer_display",
        box_col, route_col, dst_col, acct_col,
    ...
    st.dataframe(
        print_df[print_cols].style.apply(style_print, axis=1),
